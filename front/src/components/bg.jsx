...
export const Vortex = (props) => {
  ...
  // ✅ Dynamically stretch to container’s height
  const resize = (canvas) => {
    const container = containerRef.current;
    if (!container) return;
  ...
  const renderGlow = (canvas, ctx) => {
    ...
    ctx.save();
    ctx.filter = "blur(2px) brightness(100%)";
    ctx.globalCompositeOperation = "lighter";
    ctx.drawImage(canvas, 0, 0);
    ctx.restore();
  };
  ...
  useEffect(() => {
    ...
    // 🔥 Auto-resize on both window resize & content resize
    const resizeHandler = () => {
      const canvas = canvasRef.current;
    ...
    const container = containerRef.current;
    if (container) {
      const observer = new ResizeObserver(() => resize(canvasRef.current));
  ...
  return (
    <div
      className={cn("relative w-full min-h-full", props.containerClassName)}
      style={props.style}
    >
      <motion.div
        initial={{ opacity: 0 }}
